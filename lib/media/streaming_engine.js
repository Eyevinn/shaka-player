...
shaka.media.StreamingEngine.prototype.initStreams_ = function(
  ...
  var videoStream = streamsByType[ContentType.VIDEO];
  var audioStream = streamsByType[ContentType.AUDIO];
  var videoMimeType = shaka.util.MimeUtils.getFullType(
        videoStream.mimeType, videoStream.codecs);
  var audioMimeType = shaka.util.MimeUtils.getFullType(
        audioStream.mimeType, audioStream.codecs);

  /** @type {shakaExtern.SourceBufferInfo} */
  var sourceBufferInfo = {
    videoMimeType: videoMimeType,
    audioMimeType: audioMimeType,
    audioChannels: audioStream.channelsCount
  };
  var event = new shaka.util.FakeEvent('sourcebufferinitiated',
      {'detail': sourceBufferInfo});
...
shaka.media.StreamingEngine.prototype.fetch_ = function(reference) {
  ...
  var player = this.playerInterface_;
  return p.then(function(response) {
    var segmentDuration = reference.endTime - reference.startTime;

    /** @type {shakaExtern.SegmentLoadedInfo} */
    var segmentLoadedInfo = {
      loadTime: response.timeMs||null,
      size: response.data.byteLength,
      duration: segmentDuration
    };
    var event = new shaka.util.FakeEvent('segmentloaded',
        {'detail': segmentLoadedInfo});
