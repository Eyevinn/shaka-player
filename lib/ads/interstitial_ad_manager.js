...
shaka.ads.InterstitialAdManager = class {
  ...
  async resolveListMpd_(interstitial) {
    ...
    for (let i = 0; i < periods.length; i++) {
      ...
      pod.push({
        ...
        background: null,
        clickThroughUrl: null,
        tracking: this.extractCallbackTracking_(periods[i]),
      });
    }
   ...
   * @private
   */
  extractCallbackTracking_(period) {
    const byType = {};
    for (const es of shaka.util.TXml.findChildren(period, 'EventStream')) {
      if (es.attributes['schemeIdUri'] !=
          'urn:mpeg:dash:event:callback:2015') {
        continue;
      }
      for (const event of shaka.util.TXml.findChildren(es, 'Event')) {
        const url = (shaka.util.TXml.getTextContents(event) || '').trim();
        if (!url) {
          continue;
        }
