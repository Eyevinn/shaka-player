...
shaka.ads.InterstitialAdManager = class {
  ...
  addRegion(region) {
    ...
    this.resolveAlternativeMpdAndAdd_(interstitial);
  }

  /**
   * Resolves an alternative-MPD interstitial and adds the resulting
   * interstitial(s). If the @uri is a DASH List MPD (Ed.6, type="list"), it is
   * expanded into an ad pod (one interstitial per <ImportedMPD>); otherwise the
   * single interstitial is added and its @uri is played directly.
   *
   * @param {shaka.extern.AdInterstitial} interstitial
   * @private
   */
  async resolveAlternativeMpdAndAdd_(interstitial) {
    let pod = null;
    try {
      pod = await this.resolveListMpd_(interstitial);
    } catch (e) {
      shaka.log.warning(
          'Failed to resolve alternative MPD, playing the URI directly',
          interstitial.uri, e);
    }
    await this.addInterstitials(pod || [interstitial]);
  }

  /**
   * Fetches interstitial.uri and, if it is a DASH List MPD (type="list"),
   * expands its <ImportedMPD> periods into a pod of interstitials sharing a
   * groupId. Returns null if the URI is not a List MPD, in which case the
   * caller plays the URI directly.
   *
   * @param {shaka.extern.AdInterstitial} interstitial
   * @return {!Promise<?Array<shaka.extern.AdInterstitial>>}
   * @private
   */
  async resolveListMpd_(interstitial) {
    const NetworkingEngine = shaka.net.NetworkingEngine;
    const context = {
      type: NetworkingEngine.AdvancedRequestType.INTERSTITIAL_ASSET_LIST,
    };
    // Annex I (useMPDUrlQuery): carry the main MPD URL's query parameters
    // (e.g. session id) onto the alternative-MPD request for personalization.
    // The ad-decisioning server uses this to personalize the response.
    const requestUri = this.addMainMpdQuery_(interstitial.uri);
    const response =
        await this.makeAdRequest_(requestUri, context).promise;
    // Annex I (callback): the query of the List-MPD request URL (which carries the session
    // id, interests and the break/event id) is what a callback RequestParam with
    // useMPDUrlQuery="$querypart$" copies onto each beacon (see extractCallbackTracking_).
    const reqQueryIndex = requestUri.indexOf('?');
    const listMpdQuery =
        reqQueryIndex >= 0 ? requestUri.substring(reqQueryIndex + 1) : '';
    const mpd = shaka.util.TXml.parseXmlString(
        shaka.util.StringUtils.fromUTF8(response.data), 'MPD');
    if (!mpd || mpd.attributes['type'] != 'list') {
      // Not a List MPD; the @uri is a directly-playable presentation.
      return null;
    }
    const baseUri = new goog.Uri(response.uri);
    const pod = [];
    const periods = shaka.util.TXml.findChildren(mpd, 'Period');
    for (let i = 0; i < periods.length; i++) {
      const importedMpd = shaka.util.TXml.findChild(periods[i], 'ImportedMPD');
      if (!importedMpd) {
        continue;
      }
      // The imported MPD URL is the text content of the <ImportedMPD> element.
      const rawUri =
          (shaka.util.TXml.getTextContents(importedMpd) || '').trim();
      if (!rawUri) {
        continue;
      }
      const resolvedUri = baseUri.resolve(new goog.Uri(rawUri)).toString();
      pod.push({
        id: interstitial.id + '_shaka_asset_' + i,
        groupId: interstitial.id,
        startTime: interstitial.startTime,
        endTime: interstitial.endTime,
        uri: resolvedUri,
        mimeType: null,
        isSkippable: interstitial.isSkippable,
        skipOffset: interstitial.skipOffset,
        skipFor: interstitial.skipFor,
        canJump: interstitial.canJump,
        resumeOffset: interstitial.resumeOffset,
        playoutLimit: interstitial.playoutLimit,
        once: interstitial.once,
        pre: interstitial.pre,
        post: interstitial.post,
        timelineRange: interstitial.timelineRange,
        loop: false,
        overlay: null,
        displayOnBackground: false,
        currentVideo: null,
        background: null,
        clickThroughUrl: null,
        tracking: this.extractCallbackTracking_(periods[i], listMpdQuery),
      });
    }
    return pod.length ? pod : null;
  }

  /**
   * Annex I useMPDUrlQuery: returns uri with the main (base) MPD URL's query
   * parameters merged in, so a session id on the main manifest URL is carried
   * onto the alternative-MPD / ad-decisioning request.
   *
   * @param {string} uri
   * @return {string}
   * @private
   */
  addMainMpdQuery_(uri) {
    const mainUri = this.basePlayer_.getAssetUri();
    if (!mainUri) {
      return uri;
    }
    const queryIndex = mainUri.indexOf('?');
    if (queryIndex < 0) {
      return uri;
    }
    const mainQuery = mainUri.substring(queryIndex + 1);
    if (!mainQuery) {
      return uri;
    }
    return uri + (!uri.includes('?') ? '?' : '&') + mainQuery;
  }

  /**
   * Builds an AdTrackingEvent from a List-MPD Period's callback EventStream
   * (urn:mpeg:dash:event:callback:2015). The beacon URLs (Event text content)
   * are reported as impression/start tracking when the ad plays.
   *
   * @param {shaka.extern.xml.Node} period
   * @param {string=} listMpdQuery the query string of the List-MPD request URL, copied
   *   onto each beacon when a callback RequestParam (Annex I) asks for it.
   * @return {?shaka.extern.AdTrackingEvent}
   * @private
   */
  extractCallbackTracking_(period, listMpdQuery = '') {
    const TXml = shaka.util.TXml;
    const byType = {};
    for (const es of TXml.findChildren(period, 'EventStream')) {
      if (es.attributes['schemeIdUri'] !=
          'urn:mpeg:dash:event:callback:2015') {
        continue;
      }
      // Annex I (DASH Ed.6 §8.13.2.4): a RequestParam in the callback EventStream with
      // includeInRequests="callback" + useMPDUrlQuery copies the List-MPD request query
      // (session id / interests / break) onto every beacon GET, so the common (session-less)
      // beacon path stays attributable per viewer/break. queryTemplate "$querypart$" => the
      // whole query.
      let beaconQuery = '';
      if (listMpdQuery) {
        for (const rp of TXml.findChildren(es, 'RequestParam')) {
          const include =
              (rp.attributes['includeInRequests'] || 'segment').split(/\s+/);
          if (include.includes('callback') &&
              rp.attributes['useMPDUrlQuery'] == 'true') {
            beaconQuery = listMpdQuery;
            break;
          }
        }
      }
      for (const event of TXml.findChildren(es, 'Event')) {
        let url = (TXml.getTextContents(event) || '').trim();
        if (!url) {
          continue;
        }
        if (beaconQuery) {
          url += (url.includes('?') ? '&' : '?') + beaconQuery;
        }
        // Extract tracking type from URL path; group by type so each
        // beacon fires at the right point of ad playback.
    ...
    for (const type of Object.keys(byType)) {
      events.push({type: type, urls: byType[type]});
    }
    if (!events.length) {
      return null;
    }
    return shaka.ads.Utils.createTrackingFromEvents(events);
  }
  ...
  async setupVideoAd_(interstitial, sequenceLength, adPosition, initialTime,
    ...
    if (!this.playingAd_) {
      const data = (new Map())
          .set('timeOffset', timeOffset)
          .set('startedAt', this.lastTime_)
          // The break/avail event id (the Replace event id; for a List-MPD pod the ads
          // carry it as groupId). Surfaced so a UI can label the break and correlate it
          // with the beacons' event id.
          .set('eventId', interstitial.groupId || interstitial.id);
      this.sendEvent_(shaka.ads.Utils.AD_BREAK_STARTED, data);
    }
